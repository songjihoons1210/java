...
select book_id , sum( book_id * order_qty) from orders group by book_id;
-- [문제 6] '김민준'과 '이서연' 고객에 대해, 각 고객별 총 주문 수량(order_qty의 합계)을 조회하세요. 단, 총 주문 수량이 4권 이상인 고객만 조회하세요.
select customer , sum(order_qty) as 수량 from orders where customer in('김민준' , '이서연') group by customer having 수량 >= 4;
-- [문제 7] books 테이블에서 재고(stock)가 5권 미만인 도서들의 평균 가격을 조회하세요.
select title , avg(price) as 가격 from books where stock < 5 group by title ;
-- [문제 8] orders 테이블에서 3번 이상 주문된 도서의 book_id와 주문 횟수를 조회하세요.
select book_id , count(*) as 주문 from orders group by book_id having 주문 >= 3;
-- [문제 9] orders 테이블에서 총 주문 수량이 5권을 초과하는 고객의 customer와 총 주문 수량을 조회하세요.
select customer , sum(order_qty) as 주문수량 from orders group by customer having 주문수량 > 5 ;
-- [문제 10] books 테이블에서 각 저자(author)별로 출판한 도서가 2권 이상인 저자의 이름과 출판 도서 수를 조회하세요
select author , count(*) as 출판도서 from books group by author having 출판도서 >= 2;
-- [문제 11] books 테이블의 '컴퓨터' 장르 도서 중 가장 비싼 도서의 가격을 조회하세요.
select title , price from books where genre = '컴퓨터' order by price desc limit 1;
...
select count(distinct customer) as 고객 from orders ;
...
select sum(order_qty) as 총주문수량 from orders where order_date between '2023-01-01' and '2023-03-31';
-- 위 쿼리가 요구사항에 알맞습니다.
select order_date , sum(order_qty) as 총주문수량 from orders where order_date between '2023-01-01' and '2023-03-31' group by order_date;
-- [문제 18] orders 테이블에서 가장 다양한 종류의 책을 주문한 고객의 이름과 그 종류의 수를 조회하세요.
select customer , count(distinct book_id) as 종류수 from orders group by customer order by 종류수 desc limit 1; 
...
select customer as 고객 , count(distinct order_id) as 주문 from orders group by customer having 주문 = 1; -- distinct 써도 안써도 상관없음!.