...
create table test(
    ...
    mname varchar(30) not null unique , # 속성명 타입 제약조건1_제약조건2
...
# [1] insert : 레코드 삽입 , "레코드란? 테이블 내 행(가로) 단위 의 데이터들 , 1레코드 vs 1행 vs 1줄
# 주의할점 : 테이블내 데이터 삽입시 레코드 단위 , 한칸한칸에 데이터삽입이 아닌 줄/행/레코드 단위 삽입
# (1) insert 
insert into test( mno , mname , mcount ) values( 1, "유재석" , 30 );		# ;(세미콜론) 아침
# 작성한 속성명 에 대응하는 값 삽입한다.
insert into test( mcount , mno , mname) values( 40 , 2 , "강호동" );		# ctrl+enter
select * from test;
# (20 auto_increment 제약조건의 속성은 값 생략할경우 자동번호(1씩증가된) 할당
...
# (5) unique 제약조건의 속성은 중복값일 경우 삽입 불가능	Error Code: 1062. Duplicate entry '유재석' for key 'test.mname
# insert into test(mname) value ("유재석"); # 중복
# (6) 모든 속성명을 작성할 경우는 속성명 생략 가능하다.
insert into test value( 5 , "하하" , 10 );
...
# [2] SELECT : 레코드 조회
# 주의할점 : 조회 결과는 기존 테이블과 상관 없다. select만 result table 존재한다. 
...
# (3) select 속성명 from 테이블명 where 조건질 ;
...
set SQL_SAFE_UPDATES = 0 ; # safe mode 끄기 , 반대로 1켜기alter
...
delete from test where mno = 2; # 회원번호가 2와 같은 레코드 삭제
# 1. delete from 테이블명;			전체 레코그 삭제
delete from test;
