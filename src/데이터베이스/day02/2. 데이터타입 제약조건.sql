...
# [4] 현재 활성화된 데이터베이스의 테이블 목로 조회
show tables;

# 2개 이상의 속성을 정의할떄 : , 쉼표로 구분한다.
...
create table board(
    ...
    writer	varchar(3) 		# 게시물의 작성자	# private String writer
    );
select * from board; # select * from 테이블명;    

# 예2] 대기번ㄴ호발급 기록하는 테이블 설계
create table waiting(
	phone char(13)		,  # 핸드폰 번호 # 고정길이13글자  # private String phone
    count tinyint		  # 인원수     # 최대 100정도	# private byte count 
);
select * from waiting;
...
create table test4(
    ...
    필드명4 bigint auto_increment 	,	-- 지정한 속성에는 값이 생략될떄 자동으로 순서 번호 설정 , 1 2 3 4 5
    pk필드명5 double					,	-- 지정한 속성 선언
    constraint primary key ( pk필드명5 )	-- 'pk필드명5' 속성을 PK 필드로 설정
);
...
create table member(						-- [1] 회원 테이블
	mno int auto_increment , 				-- 회원번호 : 점수 , 자동번호 , *PK사용할 예정
...
select * from member;						-- 확인자 회원테이블 조회

create table board(											-- [2] 게시판 테이블
    ...
    bdate datetime default now() , 							-- 게시물작성일 :  날짜/시간 , 기본값 , noew() : 현재 날짜/시간 반환함수
    bview int default 0 ,									-- 게시물조회수 : 정수  , 기본값 , 0
    constraint primary key ( bno )	,						-- 위엣 선언된 '게시물번호'속성을 *PK필드로 설정
    mno int ,												-- 게시물 작성자 : *FK사용할 예쩡 --> PK필드 타입과 일치
...
# workbench 상단메뉴 -> [database] - [reverse engineering database] 