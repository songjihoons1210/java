...
select mid as 회원아이디 , mname 회원명 from member; -- as는 생략 가능.
select mid 회원아이디 from member as m;	 -- member 테이블명을 'm' 으로 별칭 하다. 
select mid 회원아이디 from member m;	 	 -- as는 생략 가능하다 .( 띄엄쓰끼 로 구분 )

# [2] distinct 속성명 , 속성값이 중복 인 레코드 제거
select maddr as 주소 from member;			 	-- 조ㅚ 결과 속성 값들의 중복이 존재
select distinct maddr as 주소 from member;		-- 속성 값들의 중복제거


# [3] 산술연산자 , +더하기 , - 빼기 , *곱하기 , / 나누기 , div 몫
select mnumber + 3 as 더하기 , mnumber - 3 as 빼기 ,
	 mnumber * 3 as 곱하기 , mnumber / 3 as '나누기 3',
...
select * from member where mheight <=162;									-- member테이블의 mheight속성의 값이 162 이하면
...
select * from member where mphone = ' ' ;				-- 속성값이 비어있는 표현 , ' '빈칸 대신에 null 사용한다. 
select * from member where mphone1 = null;				-- 속성값이 null 이면 [ x 불가능 ]
select * from emeber where mphone1 is null;				-- 속성값이 null 이면 [ o 가능 ]
select * from emeber where mphone1 is not null;			-- 속성값이 null 이 아니면
...
select * from member where mname like '%핑크';			-- 속성의 값이 '핑크'로 끝나는 , a핑크 , xx핑크 ,xxx핑크 - 
select * from member where mname like '%이%';			-- 속성의 값에 '이' 가 포함된 
select * from member where manme like '_이_';			-- 속성의 값에 두번째 글자가 '이' 인 세글자
# 문자패턴 : 속성명 like '문자패턴' , %:문자대응(개수상관) , _:문자대응(_수만큼)

# [5] order by 속성명 asc/desc , 정렬 , asc(오름차순/기본값) desc(내림차순)
# 오름차순 : 1 2 3 4 , a b c , ㄱ ㄴ ㄷ , 25-7-15- 25-7-16 25-07-17
# 내림차순 : 4 3 2 1 , c b a , ㄷ ㄴ ㄱ , 25-07-17 25-07-15 25-07-15
...
# 주의할점 : 2개 이상의 정렬은 , (쉼표)구분 하되 ,  1차 정렬에서 중복값이 있을경우 중복값 끼리 2차 정렬
# (1차) 먼저 주소(meddr) 내림차순으로 정렬후 (2ck) 주소가 동일한 데이터끼리 mdebut(날짜) 오름차순 정렬
...
select * from member order by mheigth desc limit 3; -- mheigth 속성값들을 내림차순 후 정렬 후 상위 3개
