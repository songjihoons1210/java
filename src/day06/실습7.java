...
public class 실습7 {
    public static void main(String[] args) {
        ...
        m1.isLogin = true;  // 멤버 클래스를 만들어  id 문자열 , 참 거짓 boolean으로 islogin 작성 boolean isLogin = false;
        ...
        // 문제 9
        MenuItem menu1 = new MenuItem(); // menuItem 라는 객체를 생성
        MenuItem menu2 = new MenuItem(); // menuItem 라는 객체를 생성
        MenuItem menu3 = new MenuItem(); // menuItem 라는 객체를 생성
        menu1.name = "김치찌개"; menu1.price = 8000; menu1.iss = true; // 첫 번째 menu1을 불러와 name , price , iss가 맞으면 true로 출력
        menu2.name = "된장찌개"; menu2.price = 8000; menu2.iss = false; // 두 번째 menu2을 불러와 name , price , iss가 맞으면 false로 출력
        menu3.name = "계란찜"; menu3.price = 3000; menu3.iss = false; // 세 번째 menu3을 불러와 name , price , iss가 맞으면 false로 출력
        if (menu1.iss == true ) { // 만약에 menu1.iss 이 ture 참이면 아래 출력
            System.out.printf("[%s] [%s]  : [%d]원  \n", menu1.name , menu1.iss , menu1.price);
        }
        if (menu2.iss) { // 만약에 menu1.iss 이 ture 참이면 아래 출력
            System.out.printf("[%s] [%s] : [%d]원  \n", menu2.name, menu2.iss, menu2.price);
        }
        if (menu3.iss) { // 만약에 menu1.iss 이 ture 참이면 아래 출력
            System.out.printf("[%s] [%s] : [%d]원  \n", menu3.name, menu3.iss, menu3.price);
        }
        // 문제10
        UserProfile profile = new UserProfile(); // userprofile라는 객체 생성
        Scanner scan = new Scanner( System.in ); // 스캐너 scan 객체 만들기
