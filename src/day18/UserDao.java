...
public class UserDao {
    ...
    public void connect() {
        try {
            ...
            System.out.println("[경고] DB 서버와 연동 실패 ");
        }
    } // func end
    ...
    public boolean userInsert2(String uname, int uage) {
        try {
            ...
            // SQL 안에 첫번째 ? 에 uname 변수값을 String 타입으로 대입
            ps.setInt(2, uage);
            // SQL 문법내 두번째 ?에 uage 변수값을 int 타입으로 대입한다.

            // 4. SQL 실행하기
            int count = ps.executeUpdate();
            // 5. SQL 결과에 따른 확인/로직/패턴
            if (count >= 1) return true; // 결과 1이면 성공
            return false; // 1 미만ㅇ ㅣ면 실패
        } catch (Exception e) {
            System.out.println(e);
    ...
    public void userSelect() {
        try {
            ...
            // ResultSet 조회결과 조작 인터페이스, import 주의 , java.sql
                ...
                // rs.getXXX( 가져올속성명 또는 순서번호 ); : 현재 레코드의 속성/명/컬럼 의 값 반환
