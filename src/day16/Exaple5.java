...
public class Exaple5 {
    public static void main(String[] args) {
        ...
        // 1. https://www.date.go.kr/index.do
        // 2. '부평구 맛있는 집' 검색
        // 3. "인천광역시 부평구_맛있는 집(맛집) 현황" 클릭
        // 4. https://www.data.go.kr/date/15103411/fileDate.do --> [다운로드]
        // 5. 현재 패키지에 지정(인천광역시 부평구_맛있는 집.csv)
        try {
            ...
            if (file.exists()) {
                ...
                byte[] bytes = new byte[(int) file.length()]; // (5) 읽어온 바이트를 저장할 바이트 배열을
                ...
                System.out.println(str.split("\n")[1].split(",")[1]); // 두번쨰 행 의 두번쨰 열 : (주)온누리푸드온누리장작구이
                // ----------------------------
                String[] 행데이터 = str.split("\n"); // 행 단위로 쪼개서 배열에 저장
                for (int i = 0 ; i < 행데이터.length ; i++) {
                    String row = 행데이터[i];
                        // System.out.println(row); // 한 줄 씪 출력
                    String[] 열데이터 = row.split(","); // 행을 열 단위로 쪼개서 배열에 저장
                        // System.out.println( 열데이터[0] ); // 업소명
