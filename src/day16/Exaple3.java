...
public class Exaple3 {
    public static void main(String[] args) {
        ...
        try {
            ...
            // (3) 읽어온 바이트 들을 지정할 배열 선언한다.
            byte[] bytes = new byte[42]; // 42바이트 를 저장할 배열 준비
            // (4) 읽어오기 , . read(바이트배열); * 무조건 (IOException) 입반예외 발생
            fin.read(bytes);
            // (5) new String( 바이트배열 ); 읽어온 바이트를 문자열로 변환 함수
            System.out.println(new String(bytes));

        }catch (FileNotFoundException e){
            System.out.println("파일 또는 경로가 존재하지 않음");
        }catch (IOException e) {
            System.out.println("입출력 도중 오류 발생 ");
        }
    }
}
